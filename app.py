...
ddb_table_name = os.getenv('DDB_TABLE_NAME', "voting-app-restaurants")
...
@app.route('/')
def home():
    html = "<h1>Welcome to the Voting App</h1><p><b>To vote, you can call the following APIs:</b></p><p>/api/outback</p><p>/api/bucadibeppo</p><p>/api/ihop</p><p>/api/chipotle</p><b>To query the votes, you can call the following APIs:</b><p>/api/getvotes</p><p>/api/getheavyvotes (this generates artificial CPU/memory load)</p>"
    return {
        "statusCode": 200,
        "body": html,
        "headers": {
...
def lambda_handler(event, context):
    ...
    if path == '/':
        ...
        response = getvotes()        
    elif path == '/api/outback':
        response = outback()
    elif path == '/api/bucadibeppo':
        response = bucadibeppo()
    elif path == '/api/ihop':
        response = ihop()
    elif path == '/api/chipotle':
        response = chipotle()
    else:
        response = {
            "statusCode": 404,
